    ...
    <div>
      ...
      <div className={`relative bg-gradient-to-br ${artist.gradient}`}>
        <div className="container-page flex flex-col items-center gap-4 py-12 text-center sm:flex-row sm:text-start">
          <span className="relative h-28 w-28 shrink-0 overflow-hidden rounded-full ring-4 ring-white/70 shadow-card">
            <Image src={artist.avatar} alt={artist.name} fill sizes="112px" className="object-cover" />
          </span>
          <div className="space-y-2">
            <h1 className="text-3xl font-bold text-slate-900 sm:text-4xl">{artist.name}</h1>
            <p className="max-w-2xl text-sm leading-relaxed text-slate-600">{artist.bio}</p>
          </div>
        </div>
      </div>

      <div className="container-page space-y-5 py-8">
        <h2 className="text-2xl font-bold text-slate-900">Songs ({videos.length})</h2>
